...
class db_output_stats:
    ...
    def __init__(self):  
        ...
        self.connection=''
        self.cursor=''
        self.fieldnames=["title","released","category","ET/PT timing and Premiere","content_defination","updated_db","Service"]
    ...
    def main(self):
        ...
        result_sheet='/operation/attachments/showtime_added_content_%s.csv'%(datetime.now().strftime('%b %d, %Y'))
        ...
        with output_file as outputcsvfile:
            ...
            query="select title,released,category,ET_PT_timing,content_defination,updated_db,Service from {table_name} where updated_db='%s'".format(table_name=db_detail.table)
