...
class send_emails:
    ...
    Titles (https://usa.newonnetflix.info/) for the recently added content NetfilxUSA service .
    ...
    def main(self):
        ...
        message["Subject"]="New on NetflixUSA Result on %s"%(datetime.now().strftime('%b %d, %Y'))
        ...
        filenames=['USA_new_on_Netflix_%s.csv'%(datetime.now().strftime('%b %d, %Y'))]
            ...
            with open(os.getcwd()+'/operation/attachments/'+file, 'r') as csvfile:
                ...
                if len(csv_dict) > 1:
                    ...
                    try:
                        ...
                        server.quit()
                        print("Email sent to %s %s"%(",".join(self.receiver_email),datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
