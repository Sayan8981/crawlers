    ...
    def get_ott_link_info(self,ott_info):
        if ott_info:
            for info in ott_info:
                self.video_info_dict=dict()
                try:
                    self.video_info_dict["urls"]= unidecode.unidecode(info["urls"]["standard_web"]).encode('utf-8').decode()
                except KeyError:
                    pass
                try:     
                    self.video_info_dict["quality"] = unidecode.unidecode(info["presentation_type"]).encode('utf-8').decode()
                except KeyError:
                    pass
                try:    
                    self.video_info_dict["currency"] = unidecode.unidecode(info["currency"]).encode('utf-8').decode()
                except KeyError:
                    pass
                try:
                    self.video_info_dict["price"]=info["retail_price"]
                    self.video_info_dict["purchase_type"] = unidecode.unidecode(info["monetization_type"]).encode('utf-8').decode()
                except KeyError:
                    pass
    ...
    def get_credits_info(self,credits_info):
        if credits_info:
            for info in credits_info:
                self.credits_dict=dict()
                self.credits_dict["role"] = unidecode.unidecode(info["role"]).encode('utf-8')
