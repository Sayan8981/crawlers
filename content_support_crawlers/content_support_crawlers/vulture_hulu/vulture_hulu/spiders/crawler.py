...
from vulture_hulu .items import *
...
class vulturehulubrowse(Spider):

    name="vulturehulubrowse"
    start_urls=["https://www.vulture.com/article/new-on-hulu-movies-shows-originals.html"]

    #initialization:
    def __init__(self):
        self.content_title=''
        self.content_title_year=''
        self.service=''
        self.content_category=''

    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(vulturehulubrowse, cls).from_crawler(crawler, *args, **kwargs)
     ...
     def content_scraped(self,response):
        ...
        for x in date_nodes:
            ...
            if '<br>' not  in title_node.extract().encode('utf-8'):
                ...
                yield Request(url=response.url,meta={"content_title":self.content_titles,"year":self.year,"content_category":self.content_category,"content_available":self.content_available,"content_defination":self.content_defination,"page_url":response.url,"service":self.service},callback=self.item_stored,dont_filter=True)
            else:
                if '<br>'    in title_node.extract():
                    t_title = title_node.xpath('.//text() | .//em//text()').extract()
                    for title in set(t_title):
                        self.content_title =  unidecode.unidecode(pinyin.get(title))
                        self.content_titles = self.get_content_title()
                        self.content_available=date.replace("Available ",'')
                        yield Request(url=response.url,meta={"content_title":self.content_titles,"year":self.year,"content_category":self.content_category,"content_available":self.content_available,"content_defination":self.content_defination,"page_url":response.url,"service":self.service},callback=self.item_stored,dont_filter=True)
    ...
    def get_content_title(self):
        ...
        elif 'Hulu Original' in self.content_title:
            self.content_defination='Hulu Original'
        else:
    ...
    def item_stored(self,response): 
        item=VultureHuluItem()
        item["title"]=response.meta["content_title"]
        item["year"]=response.meta["content_title_year"]
