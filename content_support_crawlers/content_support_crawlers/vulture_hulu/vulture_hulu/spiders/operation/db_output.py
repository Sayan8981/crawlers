...
class db_output_stats:
    ...
    def __init__(self):  
        ...
        self.connection=''
        self.cursor=''
        self.fieldnames=["title","year","content_category","updated_db"]
    ...
    def main(self):
        ...
        result_sheet='/operation/attachments/hulu_added_content_%s.csv'%(datetime.now().strftime('%b %d, %Y'))
        ...
        with output_file as outputcsvfile:
            ...
            query="select title,year,content_category,updated_db from {table_name}".format(table_name=db_detail.table)
