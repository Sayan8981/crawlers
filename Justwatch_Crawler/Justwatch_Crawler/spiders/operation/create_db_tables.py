...
class createdbtable:
    ...
    def create_require_db_tables(self):
        ...
        if db_detail.database_name not in self.db_list:
            ...
            if (db_detail.movie_table,) not in table_cursor or (db_detail.Series_table,) not in table_cursor or (db_detail.Episodes_table) not in table_cursor:
                for query in self.create_table_query:
                    self.cursor.execute(query) 
                logging.info ("table created.......")    
            else:
                logging.info ("\n")
                logging.info("Tables already exist in %s"%db_detail.database_name,table_cursor)
